...
// On ajoute le score
function addScore(username, type, number, score, userId) {
  const data = JSON.stringify({ username, type, number, score, userId });
...
score.addEventListener("click", () => {
  audioButton.play();
  const scoreLocal = Number(sessionStorage.getItem("score"));
  addScore(usernameLocal, typeLocal, lengthMusic, scoreLocal, id);
});
